...
const handleSignup = async (req, res) => {
    try {
        const { phone, password, companyName, metaId, whatsappToken } = req.body;
        ...
        const bucketName = await (0, aws_1.createCompanyBucket)(companyName);
        ...
        try {
            ...
            const companyResult = await client.query('INSERT INTO companies (name, number_id, token, prompt, sales_data_url) VALUES ($1, $2, $3, $4, $5) RETURNING id', [companyName, metaId, whatsappToken, `Tu es assistant personnel de ${companyName}`, `s3://${bucketName}`]);
            ...
            await client.query('INSERT INTO login (phone, password, company_id) VALUES ($1, $2, $3)', [phone, hashedPassword, companyId]);
...
const handleLogin = async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT l.*, c.* FROM login l JOIN companies c ON l.company_id = c.id WHERE l.phone = $1', [phone]);
...
const handleFileUpload = async (req, res) => {
    try {
        ...
        try {
            ...
            const companyResult = await client.query('SELECT sales_data_url, name FROM companies WHERE id = $1', [companyId]);
            ...
            const newFileUrl = await (0, aws_1.uploadFileToS3)(req.file, bucketName, oldFileKey);
            console.log('✓ Fichier uploadé avec succès:', newFileUrl);
            // Mettre à jour l'URL dans la base de données
            console.log('💾 Mise à jour de l\'URL dans la base de données...');
            await client.query('UPDATE companies SET sales_data_url = $1 WHERE id = $2', [newFileUrl, companyId]);
...
const handleUpdatePrompt = async (req, res) => {
    try {
        ...
        try {
            ...
            const result = await client.query('UPDATE companies SET prompt = $1 WHERE id = $2 RETURNING name, prompt', [prompt, companyId]);
...
app.get('/api/performance', async (req, res) => {
    ...
    try {
        ...
        const messagesTodayResult = await client.query(`SELECT COUNT(*) FROM conversations WHERE company_id = $1 AND updated_at >= $2`, [companyId, todayISO]);
        const messagesToday = parseInt(messagesTodayResult.rows[0].count, 10);
        // Utilisateurs actifs aujourd'hui
        const usersTodayResult = await client.query(`SELECT COUNT(DISTINCT user_whatsapp_id) FROM conversations WHERE company_id = $1 AND updated_at >= $2`, [companyId, todayISO]);
        const usersToday = parseInt(usersTodayResult.rows[0].count, 10);
        // Activité hebdomadaire (7 derniers jours)
        const weekDataResult = await client.query(`SELECT to_char(updated_at, 'Dy') as day, COUNT(*) as messages, COUNT(DISTINCT user_whatsapp_id) as users
       FROM conversations
       WHERE company_id = $1 AND updated_at >= NOW() - INTERVAL '7 days'
       GROUP BY day, to_char(updated_at, 'D')
       ORDER BY to_char(updated_at, 'D')::int`, [companyId]);
        ...
        const monthDataResult = await client.query(`SELECT to_char(updated_at, 'Mon') as month, COUNT(*) as messages, COUNT(DISTINCT user_whatsapp_id) as users
       FROM conversations
       WHERE company_id = $1 AND updated_at >= NOW() - INTERVAL '6 months'
       GROUP BY month, to_char(updated_at, 'MM')
       ORDER BY to_char(updated_at, 'MM')::int`, [companyId]);
...
app.get('/api/conversations', async (req, res) => {
    ...
    const client = await pool.connect();
    try {
        const result = await client.query(`SELECT id, user_whatsapp_id, messages, updated_at FROM conversations WHERE company_id = $1 ORDER BY updated_at DESC`, [companyId]);
